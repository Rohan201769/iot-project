...
def simulate_gear():
    ...
    for r in range(ROUNDS):
        ...
        dist_to_sink = distance(nodes['x'], nodes['y'], *SINK_POS)
        in_range = np.where(dist_to_sink < THRESH_DIST)[0]
...
st.markdown("""
GEAR (Geographic and Energy-Aware Routing) uses geographic location for forwarding.
Nodes within a certain communication range of the sink are selected to forward data.
""")
